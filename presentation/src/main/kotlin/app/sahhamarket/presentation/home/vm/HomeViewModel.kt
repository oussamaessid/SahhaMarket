...
import app.sahhamarket.domain.usecase.recipes.AddRecipesToCartUseCase
import app.sahhamarket.domain.usecase.recipes.DecrementRecipesQuantityUseCase
...
class HomeViewModel @Inject constructor(
    ...
    private val addReceiptToCartUseCase: AddRecipesToCartUseCase,
    private val decrementReceiptQuantityUseCase: DecrementRecipesQuantityUseCase,
) : ViewModel() {
    ...
    fun processAction(action: UiAction) {
        viewModelScope.launch {
            when (action) {
                ...
                is UiAction.OnAddToCart -> addReceiptToCart(action.receipt)
                is UiAction.OnLessQuantityReceipt -> decrementReceiptQuantity(action.receipt)
                ...
                is UiAction.NavigateToRecipeDetail -> {
                    _uiEvent.emit(UiEvent.NavigateToRecipeDetail)
                }
    ...
    private suspend fun addReceiptToCart(receipt: Receipt) {
        addReceiptToCartUseCase(receipt)
    }

    private suspend fun decrementReceiptQuantity(receipt: Receipt) {
        decrementReceiptQuantityUseCase(receipt)
    }
    ...
    sealed interface UiAction {
        ...
        data class OnAddToCart(val receipt: Receipt) : UiAction
        data class OnLessQuantityReceipt(val receipt: Receipt) : UiAction
        ...
        data object NavigateToRecipeDetail : UiAction
    ...
    sealed interface UiEvent {
        ...
        data object NavigateToRecipeDetail: UiEvent
