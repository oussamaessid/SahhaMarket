    ...
    LaunchedEffect(Unit) {
        viewModel.uiEvent.collectLatest { event ->
            when (event) {
                ...
                is PackagesViewModel.UiEvent.NavigateToPackageDetails -> {
                    onNavigateToDetails(event.packageId)
                }
            }
        }
    }
                        ...
                        Section(title = "Weekly Packages") {
                            LazyRow {
                                items(state.weeklyPackages) { item ->
                                    PackageCard(
                                        packageItem = item,
                                        onClick = { viewModel.processAction(PackagesViewModel.UiAction.OnPackageClick(item.id)) }
                                    )
                                }
                            }
                        }
